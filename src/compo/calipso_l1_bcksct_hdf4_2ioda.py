...
"""
Python code to ingest HDF4 CALIPSO L1 APro data
"""
...
GlobalAttrs = {
    ...
    "platformLongDescription": "CALIPSO L1 Lidar Data",
}
...
# V2 nlocs_int32 = np.array(len(obs_data[('latitude', 'MetaData')]), dtype='int32')
    ...
    VarDims = {
        ...
        'QC_Flag_2': ['nlocs'],
        'Pressure': ['nlocs', 'nmet_levs'],
        'Temperature': ['nlocs', 'nmet_levs'],
        'level':['nlevs'],
        'metlevel':['nmet_levs'],
    ...
    DimDict = {
        ...
        'nmet_levs': len(obs_data[('metlevel','MetaData')]),
    }
    print(DimDict)
    ...
    VarAttrs[('QC_Flag',   'ObsValue')]['_FillValue'] = int_missing_value
    VarAttrs[('QC_Flag_2',  'ObsValue')]['_FillValue'] = int_missing_value
...
def get_data(f, obs_data, meta_dict):
    ...
    #nchans = 2
    nlevs = len(meta_dict['Lidar_Data_Altitudes'])
    print( 'nlevs',nlevs)
    nmet_levs = f.select('Pressure').get().shape[1]
    print( 'Pressure shape', f.select('Pressure').get().shape )
    
    obs_data[('latitude', 'MetaData')] = np.array(f.select('Latitude').get()[:,0], dtype='float32')
    obs_data[('longitude', 'MetaData')] = np.array(f.select('Longitude').get()[:,0], dtype='float32')
    obs_data[('level', 'MetaData')] = np.array(np.arange(nlevs)+1, dtype='int32')
    obs_data[('metlevel', 'MetaData')] = np.array(np.arange(nmet_levs)+1, dtype='int32')
    ...
    obs_data[('BackscatterCoeff_532', "ObsValue")] = np.array(f.select("Total_Attenuated_Backscatter_532").get(),dtype='float32')
    obs_data[('BackscatterCoeff_1064',"ObsValue")] = np.array(f.select("Attenuated_Backscatter_1064").get(),dtype='float32')
    # revisit the observation error of backscatter coefficient
